...
pc_ip="10.14.21.20"
getparam={'parama': '54010000001', 'paramb': '电脑', 'paramc': '即享贷D', 'paramd': '4000', 'parame': '1000', 'paramf': '全部', 'paramg': '12', 'paramh': '台式电脑', 'parami': '三星', 'paramj': '不参加', 'paramk': '轮循', 'paraml': 's'}
uname="536824"
markval="536824_3_23"
class jyb_order_536824_3_1(unittest.TestCase):
    @classmethod
    ...
    def tests_pr(self):
        ...
        A = MysqlUtil()
        time.sleep(2)
        A.mysql_execute("INSERT INTO mobiletest_mobiledata (contract_no,con_name,con_ident,con_phone,create_time,capital_source,con_status,username,runnum) VALUES ('%s','%s','%s','%s',NOW(),'%s','%s','%s','%s')"%(cnoval,textval,idcard,mobilephone,getparam['paramk'],uname,markval))
    ...
    def tests_r(self): #流程测试r合同
        ...
        if com.findItem("基本信息"):   #流程测试r合同
            ...
            MysqlUtil().mysql_execute("UPDATE mobiletest_mobiledata SET con_status='r' WHERE con_name='%s'"%newcon_name)    #流程测试r合同
        else:   #流程测试r合同
    ...
    def tests_s(self):  #流程测试s合同
        ...
        com3.elm_operate(15, "")  #流程测试s合同
        time.sleep(4)  #流程测试s合同
        MysqlUtiltwo().mysql_execute("update credit_bankcard_four set check_result=2000 where name='%s'"%newcon_name)  #流程测试s合同
        ...
        if com3.findItem("绑定银行卡"):  #流程测试s合同
            send_codee2 =MysqlUtiltwo().mysql_getstring("SELECT t.sms_code  FROM (SELECT * FROM sms_verify_info WHERE phone = '%s' ORDER BY sent_time DESC) t  LIMIT 1"%mobilephone)  #流程测试s合同
        ...
        if com3.findItem("成功提交"):   #流程测试s合同
            ...
            MysqlUtil().mysql_execute("UPDATE mobiletest_mobiledata SET con_status='s' WHERE con_name='%s'"%newcon_name)    #流程测试s合同
        else:   #流程测试s合同
